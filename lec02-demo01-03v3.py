...
# Last modified on Tue Mar 9, 2021 19:14
...
v   = -1.0  # advection velocity (can be positive or negative)
...
scheme_color = [ 'r', 'orange', 'y', 'brown', 'b', 'g','m']
scheme_name = ['FTCS', 'Lax', 'upwind', 'downwind', 'Lax-Wendroff (2-step)', 'Lax-Wendroff (1-step)', 'Matsuno']
...
animat_play = False             # DECIDE whether to play the animation
...
scheme_ani = 5                  # SELECT one scheme to display in the animation {0,1,2,3,4}
scheme_err_plot = [0,1,2,3,4,5,6]	# SELECT the scheme(s) that displays in the error-vs-time plot {0,1,2,3,4}
...
#       (3) upwind scheme (conditionally stable)
            if v > 0:
                u[2][i] = u_in[2][i] - alpha*( u_in[2][i] - u_in[2][im] )
            if v < 0:
                u[2][i] = u_in[2][i] - alpha*( u_in[2][ip] - u_in[2][i] )

#       (4) downwind scheme (unconditionally unstable)
            if v > 0:
                u[3][i] = u_in[3][i] - alpha*( u_in[3][ip] - u_in[3][i] )
            if v < 0:
                u[3][i] = u_in[3][i] - alpha*( u_in[3][i] - u_in[3][im] )
...
#		(5) the Richtmyer two-step Lax-Wendroff scheme
        ...
        if v > 0:
            ...
            for i in range( N ):
                ...
                u[4][i] = u_in[4][i] - alpha*( u_half[i] - u_half[im] )
        if v < 0:
            for i in range( N ):
                im = boundary_cond( i, N, func_sel )[1]
            ...
            for i in range( N ):
                ip = boundary_cond( i, N, func_sel )[0]
                u[4][i] = u_in[4][i] - alpha*( u_half[ip] - u_half[i] )

#       (6) the one step (?) Lax-Wendroff scheme
        for i in range( N ):
            ip, im = boundary_cond( i, N, func_sel )
            ip2 = boundary_cond( ip, N, func_sel )[0]
            im2 = boundary_cond( im, N, func_sel )[1]
            u[5][i] = u_in[5][i] - 0.5*alpha*( u_in[5][ip] - u_in[5][im] ) + 0.5*(alpha**2)*( u_in[5][ip2] - 2*u_in[5][i] + u_in[5][im2] )

#		(7) the Matsuno (or Euler-backward) scheme
        for i in range( N ):
            ip, im = boundary_cond( i, N, func_sel )
            ip2 = boundary_cond( ip, N, func_sel )[0]
            im2 = boundary_cond( im, N, func_sel )[1]
            u[6][i] = u_in[6][i] - 0.5*alpha*( u_in[6][ip] - u_in[6][im] ) + (0.5*alpha)**2*( u_in[6][ip2] - 2*u_in[6][i] + u_in[6][im2] )
...
# Evaluate the integrals as approximated by trapezoidal rule.
    for S in scheme_err_plot:
        integ_norm = np.trapz(u[S]) / np.trapz(u_ref)  # Assuming density is even everywhere
        int_data[S].append( integ_norm )
...
if err_time_plot == True:
    ...
    ax1.set_ylim(1.0e-10, 1.0e10)
    ax1.set_title('Error vs. time \n (intial condition = %s)' % func_types[func_sel])
    ax1.grid()
    for S in scheme_err_plot:
    ...
    ax2 = fig2.add_subplot( xlabel = 'Time $t$', ylabel = 'Integral')
    ax2.set_xlim(0.0, end_time)
    ax2.set_ylim(0, 2)
    ax2.grid()
    ax2.set_title('Normalized trapezoidal integral vs. time \n (intial condition = %s)' % func_types[func_sel])
    for S in scheme_err_plot:
        ax2.plot( t_data, int_data[S], ls = '--' if (S == 5 or S == 6) else None,
    			 label = 'Scheme %i: ' % (S+1) + scheme_name[S], color = scheme_color[S] )
