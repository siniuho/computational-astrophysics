...
fig.suptitle('DFT for Diffusion', fontsize=14)
...
##        assuming periodic boundary condition
         ...
         im = (i-1+N) % N
#
         u1[i] = u_in[i] + dt*D*( u_in[ip] - 2*u_in[i] + u_in[im] )/dx**2.0
...
#     inverse DFT
      u2  = np.fft.irfft( uk - dt * D * k**2.0 * uk  )
...
#  plot
   ...
   print('t/T = %7.4f, error = %10.3e' % (t/t_scale, err1, err2))
