...
#  periodic boundary condition
   U[0:nghost]   = U[N_In:nghost+N_In] # left ghost zone
   U[N-nghost:N] = U[N-nghost-N_In:N-N_In] # right ghost zone
...
# d = density = ρ,
# px = x-momentum = ρu, py = y-momentum = ρv, pz = z-momentum = ρw
# e = total energy density
# Pressure P = (e - ½(px²+py²+pz²)/ρ)*(γ-1)
...
def ComputeTimestep( U ):
   P = ComputePressure( U[:,0], U[:,1], U[:,2], U[:,3], U[:,4] ) # of each cell
   a = ( gamma*P/U[:,0] )**0.5 # sound speed of each cell: a = sqrt(γP/ρ) 
   u = np.abs( U[:,1]/U[:,0] ) # abs val of x-velocity of each cell: u = ρu/ρ
   v = np.abs( U[:,2]/U[:,0] ) # abs val of y-velocity of each cell: v = ρv/ρ
   w = np.abs( U[:,3]/U[:,0] ) # abs val of z-velocity of each cell: w = ρw/ρ

#  maximum information speed in 3D
   max_info_speed = np.amax( u + a ) # find the fastest speed at which the matter propagates
   dt_cfl         = cfl*dx/max_info_speed # Courant number cfl = speed*(Δt/Δx)
   dt_end         = end_time - t # remaining time

   return min( dt_cfl, dt_end ) # return the smaller one  
...
# L: left face-centered value;     C: cell-centered value;     R: right face-centered value
...
# van-Leer limiter: if slope_L and slope_R are of same signs, put their harmonic mean; if opposite signs, put 0.
   slope_limited = np.where( slope_LR>0.0, 2.0*slope_LR/(slope_L+slope_R), 0.0 ) 

# minmod limiter
#   slope_limited = np.where( slope_LR > 0.0, np.sign(slope_R) * np.minimum( np.abs(slope_R), np.abs(slope_L) ), 0.0 ) 
   
# superbee limiter
#   slope_limited01 = np.where( slope_LR > 0.0, np.sign(slope_R) * np.minimum( np.abs(slope_R), 2*np.abs(slope_L) ), 0.0 )
#   slope_limited02 = np.where( slope_LR > 0.0, np.sign(slope_R) * np.minimum( 2*np.abs(slope_R), np.abs(slope_L) ), 0.0 )
#   slope_limited = np.where( slope_LR > 0.0, np.sign(slope_limited01) * np.maximum( np.abs(slope_limited01), np.abs(slope_limited02) ), 0.0 )

# numpy.where(condition, x, y), x and y are array-like
# When the condition is true, put the value of Array x, otherwise put the value of Array y.
...
def Conserved2Primitive( U ):
   W = np.empty( 5 )
# Conserved variables U = [ρ, ρu, ρv, ρw, E]
# Primitive variables W = [ρ, u, v, w, P]
   W[0] = U[0]
   W[1] = U[1]/U[0]
   W[2] = U[2]/U[0]
   W[3] = U[3]/U[0]
...
#  allocate memory
   W = np.empty( (N,5) ) # vector for "cell-centered" primitive variables
   L = np.empty( (N,5) ) # vector for "left face-centered" primitive variables
   R = np.empty( (N,5) ) # vector for "right face-centered"primitive variables
...
#     convert primitive variables --> conserved variables
      L[j] = Primitive2Conserved( L[j] ) #  "left face-centered" primitive variables
...
fig.suptitle('acoustic wave / MUSCL-Hancock scheme')
